...
%bciDatasetList = {'E20200316','E20200317','E20200318','N20171215','N20180221','R20201020','R20201021'};
    reachDatasetList = {'E20210706','E20210707','E20210708','E20210709','E20210710'...
     ...
     bciDatasetList = {'E20200316','E20200317','E20200318','E20210901','N20171215','N20180221','R20201020'};
    task = 'bci';
    
    for datasetList = reachDatasetList%bciDatasetList%{'E20210901'}%{'E20200316'}%%
        %Load data
        ...
        elseif numPdims==3 && numTdims==2 %target var captured by "best" posture dims
            [bestPdims,~,~,~,explained_bestP,~] = pca(targetMargTraj*DP); 
            vPP = diag(DP'*CP*DP)./trace(CP);
            vPP = vPP(1)+vPP(2);
            vPT = diag(DT'*CP*DT)./(vPP*trace(CP));
            vTT = diag(DT'*CT*DT)./trace(CT);
            vTP = var(targetMargTraj*DP*bestPdims(:,1:2))./trace(CT); 
            
            APT = trace(DT'*CP*DT)./(vPP*trace(CP));
            ATP =  trace(cov(targetMargTraj*DP*bestPdims(:,1:2)))/trace(DT'*CT*DT);
        end
        ...
        for i = 1:numReps
            if numPdims == 2 && numTdims == 2
                ...
                nullAPT(i) = APT;
                nullATP(i) = ATP;
            elseif numPdims==3 && numTdims==2
                v1 = normrnd(0,1,numChannels,2);
                v2 = normrnd(0,1,numChannels,2);
                DR = orth([v1,v2]);
                APT = trace(DR'*CP*DR)/(vPP*trace(CP));      
                ATP = trace(DR'*CT*DR)/trace(DT'*CT*DT);
                nullAPT(i) = APT;
                nullATP(i) = ATP;
            elseif numPdims==1 && numTdims==2
                ...
                DR = orth(v);
                APT = trace(DR'*CP*DR)/trace(DP'*CP*DP);
                ATP = trace(DR'*CT*DR)/(vTT*trace(CT));
                nullAPT(i) = APT;
                nullATP(i) = ATP;
...
%                 end
            ...
            switch dataset
                case {'E20210901'}
                    resultStruct(structInd).animal = 'E2';
                otherwise

...
%% Create CNOTrajStruct
    cnoTrajStruct = struct('posture',[],'target',[],'traj',[]);
    trajStructInd = 1;
    for posture = postureList
        randNum = rand;
        for target = targetList
            %Get posture offset
            postureOffset = [0,0.4*posture,0];
            %Get rotation matrix
            rotMat = rotX(180*(target-1));
            if target == 2
                rotMat = rotMat*rotY(180);
            ...
            stretchMat = stretchFac.*eye(3);
            %Fill orgTrajStruct
            cnoTrajStruct(trajStructInd).posture = posture;
            cnoTrajStruct(trajStructInd).target = target;
            cnoTrajStruct(trajStructInd).traj = baseTraj*stretchMat*rotMat + postureOffset;
            trajStructInd = trajStructInd + 1;
        end
    end    
    
%% Create ncoTrajStruct
    ncoTrajStruct = struct('posture',[],'target',[],'traj',[]);
    trajStructInd = 1;
    for posture = postureList
        randNum = rand;
        for target = targetList
            ...
            stretchMat = stretchFac.*eye(3);
            %Fill orgTrajStruct
            ncoTrajStruct(trajStructInd).posture = posture;
            ncoTrajStruct(trajStructInd).target = target;
            ncoTrajStruct(trajStructInd).traj = baseTraj*stretchMat*rotMat + postureOffset;
            trajStructInd = trajStructInd + 1;
        end
    end
...
set(gca,'fontsize',fs)
if saveFig
    saveas(gcf,fullfile(saveDir,'orgv2Hyp.svg'));
end
...
set(gca,'fontsize',fs)
if saveFig
    saveas(gcf,fullfile(saveDir,'ncnoOrgHyp.svg'));
end
...
for posture = postureList
    for target = targetList
        traj = ncoTrajStruct([ncoTrajStruct.posture]==posture & [ncoTrajStruct.target]==target).traj;
        plot3(traj(:,xDim),traj(:,yDim),traj(:,zDim),'Color',pcmap(posture,:),'LineWidth',lw)
        plot3(traj(1,xDim),traj(1,yDim),traj(1,zDim),'.','MarkerEdgeColor',pcmap(posture,:),'MarkerSize',ds);
        plot3(traj(end,xDim),traj(end,yDim),traj(end,zDim),'v','MarkerEdgeColor',pcmap(posture,:),'MarkerFaceColor',pcmap(posture,:),'MarkerSize',as);
    end
...
set(gca,'fontsize',fs)
if saveFig
    saveas(gcf,fullfile(saveDir,'ncoOrgHyp.svg'));
end
...
for posture = postureList
    for target = targetList
        traj = cnoTrajStruct([cnoTrajStruct.posture]==posture & [cnoTrajStruct.target]==target).traj;
        plot3(traj(:,xDim),traj(:,yDim),traj(:,zDim),'Color',pcmap(posture,:),'LineWidth',lw)
        plot3(traj(1,xDim),traj(1,yDim),traj(1,zDim),'.','MarkerEdgeColor',pcmap(posture,:),'MarkerSize',ds);
        plot3(traj(end,xDim),traj(end,yDim),traj(end,zDim),'v','MarkerEdgeColor',pcmap(posture,:),'MarkerFaceColor',pcmap(posture,:),'MarkerSize',as);
    end
...
set(gca,'fontsize',fs)
if saveFig
    saveas(gcf,fullfile(saveDir,'cnoOrgHyp.svg'));
end

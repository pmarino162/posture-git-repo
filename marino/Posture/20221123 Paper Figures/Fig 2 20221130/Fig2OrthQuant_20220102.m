...
%% Setup resultStruct
    resultStruct = struct('animal',[],'dataset',[],'vPP',[],'vPT',[],'vTT',[],'vTP',[],'APT',[],'ATP',[],'nullDist',[]);
    structInd = 1;
...
%             end
        ...
        [Ua,Sa] = eig(Ca);
        d = 1;
        while sum(explained(1:d)) < 80
            d = d + 1;
        end
        numReps = 1000;
        Achance = NaN(1,numReps);
        for i = 1:numReps
            if numPdims == 2 && numTdims == 2
                v = normrnd(0,1,numChannels,2);
                v1 = orth((Ua*sqrt(Sa)*v)/norm(Ua*sqrt(Sa)*v));
                v = normrnd(0,1,numChannels,2);
                v2 = orth((Ua*sqrt(Sa)*v)/norm(Ua*sqrt(Sa)*v));
                Achance(i) = trace(v2'*(v1*v1')*v2)/d;
            else
%                 v = normrnd(0,1,numChannels,1);
%                 v1 = orth((Ua*sqrt(Sa)*v)/norm(Ua*sqrt(Sa)*v));
%                 v = normrnd(0,1,numChannels,2);
%                 v2 = orth((Ua*sqrt(Sa)*v)/norm(Ua*sqrt(Sa)*v));
%                 projV1 = v1'*v2*v2';
%                 projV1 = projV1/norm(projV1);
%                 Achance(i) = trace(v2'*(projV1*projV1')*v2)/d;     
...
%                 end
            ...
            resultStruct(structInd).nullDist = Achance;
...
%% Plot Results
    ...
    for monkey = monkeyList
       ...
       %Plot Data
       offset = 1/2;
       scatter(monkeyInd*2*ones(1,numSamp)-offset,APT,100,scmap(6,:),'filled','MarkerFaceAlpha',0.5);
       scatter(monkeyInd*2*ones(1,numSamp),ATP,100,scmap(6,:),'filled','MarkerFaceAlpha',0.5);
       nullDist = horzcat(tempResultStruct.nullDist);
       nullMean = mean(nullDist);
       nullStd = std(nullDist);
       %Plot errorbar
       e = errorbar(monkeyInd*2+offset,nullMean,nullStd,'ok','LineWidth',2,'MarkerSize',2,'MarkerFaceColor','k');
       alpha = 0.3;
       set([e.Bar, e.Line], 'ColorType', 'truecoloralpha', 'ColorData', [e.Line.ColorData(1:3); 255*alpha])
       
       %Update xtickList
       xtickList(xtickListInd) = monkeyInd*2 - offset;
       xtickList(xtickListInd+1) = monkeyInd*2;
       xtickList(xtickListInd+2) = monkeyInd*2 + offset;

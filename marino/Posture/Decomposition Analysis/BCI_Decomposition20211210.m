...
%% Perform posture LDA on reaching neural activity numDims
%     trajStruct = holdTrajStruct([holdTrajStruct.target]==3);
%     trajStruct = trajStruct(ismember([trajStruct.posture],[1:7]));
    ...
    for posture = 1:numPostures
       allObs = [];
       postureTrajStruct = tempTrajStruct([tempTrajStruct.posture]==posture);
       for i = 1:size(postureTrajStruct,2)
          allObs = vertcat(allObs,postureTrajStruct(i).allSmoothFR.traj); 
       end
       numObs = size(allObs,1);
       obsStruct(posture).posture = posture; 
       obsStruct(posture).numObs = numObs; 
       obsStruct(posture).allObs = allObs; 
    end
    ...
    for target = targetList
       ...
       for i = 1:size(tempTrajStruct,2)
           ...
           for j = 1:numTraj
               ...
               reachObsStruct(structInd).allObs(allObsInd:allObsInd+2,:) = tempTrajStruct(i).allSmoothFR(j).traj(zeroInd+2:zeroInd+4,:);
               allObsInd = allObsInd + 3;
           end
       end

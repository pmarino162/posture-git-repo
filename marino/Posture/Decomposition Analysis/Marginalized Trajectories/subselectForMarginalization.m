function [Data] = subselectForMarginalization(Data,dataType)
...
switch dataType
    ...
    case 'BCI'
        ...
        figure
        target = [trialCounts.target]';
        posture = [trialCounts.posture]';
        numTrials = [trialCounts.numTrials]';
        tbl = table(target,posture,numTrials);
        heatmap(tbl,'target','posture','ColorVariable','numTrials')
...
%         close all
    ...
    case 'reaching'
        ...
        %Remove targets 2,4,6,8
%         rmCond = ismember([trialCounts.target],[2,4,6,8]);
        rmCond = ismember([trialCounts.posture],[6]);
        rmTrialNums = [trialCounts(rmCond).trialNum];
        trialNum = [Data.trialNum];
        Data(ismember(trialNum,rmTrialNums)) = [];
        close all

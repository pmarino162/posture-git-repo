...
%% Run GPFA On States of Interest; Save projection to Traj Field
    ...
    for trial = 1:numTrials
        ...
        binStartTime = trialInclStates(1).addTimeToBeginning{1}+binWidth/2;
...
%% Get "postural component" from trajectory IC's 
    ...
    for posture = 1:5
        ...
        for i = 1:numAvgTraj
            ICs(i,:) = tempData(i).avgStep1GPFA.traj(1,:);
        end
...
%% Get "target component" from target average trajectories
    ...
    for target = 1:8
        ...
        for i = 1:numAvgTraj
           trajLengths(i) = size(tempData(i).avgStep1GPFA.traj,1); 
        end
        allTargTraj = nan(max(trajLengths),numDims,numAvgTraj);
        for i = 1:numAvgTraj
            allTargTraj(1:trajLengths(i),:,i) = tempData(i).avgStep1GPFA.traj;
        end
...
%% Use condition-averaged trajectories to perform posture, target, and PxT LDA
    ...
    for i = 1:size(trajStruct,2)
        ...
        numPoints = size(trajStruct(i).avgStep1GPFA.traj,1);
...
%% Add LDA Projections to Traj Struct and Model Struct
    for i = 1:size(trajStruct,2)
       trajStruct(i).avgStep1PostureLDA = trajStruct(i).avgStep1GPFA.traj*postureLDA;
       trajStruct(i).avgStep1TargetLDA = trajStruct(i).avgStep1GPFA.traj*targetLDA;
       trajStruct(i).avgStep1PostTargOrth = trajStruct(i).avgStep1GPFA.traj*postTargOrth;
       trajStruct(i).avgStep1PxTLDA = trajStruct(i).avgStep1GPFA.traj*pxtLDA;
    end
...
%% Visualize Model and Actual Trajectories 
    ...
    for posture = 1:5
        for target = 1:8
            traj = trajStruct(find([trajStruct.posture]==posture & [trajStruct.target]==target)).avgStep1GPFA.traj;

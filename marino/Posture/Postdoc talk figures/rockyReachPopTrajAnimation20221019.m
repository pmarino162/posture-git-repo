...
for datasetList = {'R20200222'}%{'E20210706'}%,'N20190306','R20200222'}
    %Load data
...
%     [CPOrth,~] = qr([postureLDA(:,1:2),CIPCA(:,1)]); CPOrth = CPOrth(:,1:3);
    ...
    for frame = frameList
        for posture = postureList
            for target = targetList
                if any([trajStruct.posture]==posture & [trajStruct.target]==target)
                    if frame ~= 1
                        traj = trajStruct([trajStruct.posture]==posture & [trajStruct.target]==target).avgSmoothFR.traj(timePts,:);
    ...
    %Reach traj eminating animation - posture 2

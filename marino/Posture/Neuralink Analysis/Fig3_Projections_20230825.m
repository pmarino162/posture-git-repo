...
%% Run loop for each dataset      
    ...
    targetMargTraj = squeeze(targetMargTraj);
        ...
        [targetPCs,~,~,~,targetExplained,targetMargMu] = pca(targetMargTraj); 
    ...
    postureMargTraj = squeeze(postureMargTraj);
        ...
        [posturePCs,~,~,~,postureExplained,postureMargMu] = pca(postureMargTraj); 
    ...
    numPosturePCs = 2;
       ...
       [PTOrth,~] = qr([posturePCs(:,1:2),targetPCs(:,1)]); PTOrth = PTOrth(:,1:3);    
        ...
        for target = targetList
            ...
            plot(targetProj(:,1),targetProj(:,2),'LineWidth',2,'Color',tcmap(target,:));
            targetInd = targetInd + 1;
        end
       ...
       %EigenSpectra
        ...
        sum(targetExplained(1:2))
        
        figure 
        bar(postureExplained(1:10,1))
        xlabel('PC')
        ylabel('Variance Explained (%)')
        ax = gca;
        ax.FontName = 'arial';

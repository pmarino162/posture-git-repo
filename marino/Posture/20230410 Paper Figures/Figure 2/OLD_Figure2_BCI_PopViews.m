...
%% Setup saveFig   
    ...
    saveDir = 'C:\Users\pmari\OneDrive - University of Pittsburgh\Documents\Posture\Paper\20230410\Figure 2\orthographic projections';
...
%% Run loop for each dataset    
    ...
    for datasetList = {'E20210901'}%{'R20201020'}%{'E20200317','E20210901','N20180221'} 
...
%             trajStruct(i).CPOrth.traj = trajStruct(i).avgSmoothFR.traj*CPOrth;
        ...
        set(gca,'fontsize',fs)
        if saveFig
            saveas(gcf,fullfile(saveDir,[dataset,'_PTOrth_PCA_LDA.svg']));
            saveas(gcf,fullfile(saveDir,[dataset,'_PTOrth_PCA_LDA.fig']));
        end
        %axis equal


        %Plot - Orthographic - LDA
        figure
        hold on
        timePts = 1:minNumTimestamps;
        xDim = 2; yDim = 3; zDim = 1;
        ...
        set(gca,'fontsize',fs)
        if saveFig
            saveas(gcf,fullfile(saveDir,[dataset,'_PTOrth_LDA.svg']));
            saveas(gcf,fullfile(saveDir,[dataset,'_PTOrth_LDA.fig']));
        end
        
        %Plot - Orthographic - PCA
        figure
        hold on
        timePts = 1:minNumTimestamps;
        xDim = 2; yDim = 3; zDim = 1;
        ...
        set(gca,'fontsize',fs)
        if saveFig
            saveas(gcf,fullfile(saveDir,[dataset,'_PTOrth_PCA.svg']));
            saveas(gcf,fullfile(saveDir,[dataset,'_PTOrth_PCA.fig']));
        end

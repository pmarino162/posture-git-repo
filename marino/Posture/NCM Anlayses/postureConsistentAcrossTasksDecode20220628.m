...
%% Load Data, subselect
    ...
    trialInclStates(2).trialName = {'HC_CenterOut_ForceBar_20200314'};
        trialInclStates(2).inclStates = {{'kin','moveOnsetTime','first',-200},{'kin','moveOnsetTime','first',-50}};
    trialInclStates(3).trialName = {'IsometricForce_1D'};
        trialInclStates(3).inclStates = {{'state','Target','first',50},{'state','Target Hold','first',0}};
...
%         %All points
%         for j = 1:size(trajStruct(i).allSmoothFR,2)
%            trajStruct(i).allSmoothFR(j).obs = trajStruct(i).allSmoothFR(j).traj; 
%         end
        %Condition average
...
%% Create obsStruct for across task classification
    %Get mininum number of trials for task*target*posture condition

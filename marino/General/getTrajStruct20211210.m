function [trajStruct] = getTrajStruct20211210(Data,condFields,trajFields,trialInclStates,binWidth,varargin)
...
%% Fill trajStruct
    ...
    for condInd = 1:numCond
        ...
        for trial = 1:numCondTrials
            ...
            for trajField = 1:numTrajFields
               trajFieldName = trajFields{trajField};
               [traj,timestamps] = getStatesTraj20211210(tempData(trial),trialInclStates,trajFieldName,binWidth,'chMeans',chMeans,'chStd',chStd,'rmCh',rmCh);
               upperTrajFieldName = [upper(trajFieldName(1)),trajFieldName(2:end)];
    ...
    for condInd = 1:numCond
        ...
        for trajField = 1:numTrajFields
            trajFieldName = trajFields{trajField};
            upperTrajFieldName = [upper(trajFieldName(1)),trajFieldName(2:end)];
            [avgTraj,timestamps] = getAvgTraj20211210(trajStruct(structInd).(['all',upperTrajFieldName]),binWidth);

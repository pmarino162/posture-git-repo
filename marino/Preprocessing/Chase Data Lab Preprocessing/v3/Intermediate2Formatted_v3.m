...
% project
    ...
    INIT_trial_type = InitStartData(IncrState, 'TrialType', []);
    ...
    if ~isfield(iData.QL.Data, 'END_TASK_STATE')
        ...
        fprintf('Not END_TASK_STATE message...\n');
        Data = [];
        return;
    end        
    ...
    config_name_idx = strfind(lower(config_dir), 'config');
    ...
    trial_type_per_bin = zeros(1, num_samples);
    if isfield(D.TRIAL_CONFIG, 'trial_config_index')
        trial_type_per_bin = FillValueAtInds(D.TRIAL_CONFIG.trial_config_index(tc_idx), ...
                        trial_start_to_SG.idx, num_samples, INIT_trial_type);   
    end
...
%     end
    ...
    if( isfield( D, 'VR_CONTROL_SPACE_ACTUAL_STATE'))
        position.Actual = D.VR_CONTROL_SPACE_ACTUAL_STATE.pos;
        velocity.Actual = D.VR_CONTROL_SPACE_ACTUAL_STATE.vel;
        %position.ActualCoriMatrix = D.VR_CONTROL_SPACE_ACTUAL_STATE.CoriMatrix;
    end
    ...
    elseif ( isfield( D, 'HVR_SET_CURSOR_POSITION'))
        position.Control = [D.HVR_SET_CURSOR_POSITION.x; 
                            D.HVR_SET_CURSOR_POSITION.y; 
                            D.HVR_SET_CURSOR_POSITION.z];
    end
    ...
    if( isfield( D, 'VR_CONTROL_CONFIG'))
    ...
    %Raw spikes stuff
        Data.rawSpikes.channel = D.SPIKE_SNIPPET.ss.channel;
        Data.rawSpikes.unit = D.SPIKE_SNIPPET.ss.unit;
        Data.rawSpikes.source_index = D.SPIKE_SNIPPET.ss.source_index;
        Data.rawSpikes.timestamps = D.SPIKE_SNIPPET.ss.source_timestamp;
        Data.rawSpikes.binSourceTimestamps = D.SAMPLE_GENERATED.source_timestamp;
        Data.rawSpikes.binSourceTimestampsAligned = time_per_bin;
    Data.Time = time_per_bin;
    Data.TrialNo = trial_no_per_bin;
    Data.BlockNo = block_no_per_bin;
    Data.TrialType = trial_type_per_bin;

...
%% Reformat Chase Lab Spikes struct to match Batista Lab format
    spikes = struct('timestamps',[],'channel',[],'sort',[]);
    rawSpikesFieldNames = fieldnames(rawSpikes);
    numFieldNames = size(rawSpikesFieldNames,1);
    tempChannelList = zeros(1,numFieldNames);
    for i = 1:numFieldNames
        tempChannelList(i) = str2num(rawSpikesFieldNames{i,1}(5:6));
    end
    uniqueChannelList = unique(tempChannelList);
    allPossibleChannels = 1:96;
    numMissingChannels = size(setdiff(allPossibleChannels,uniqueChannelList),2);
    spikes = repmat(spikes,1,numFieldNames+numMissingChannels);
    structInd = 1;
    for channel = allPossibleChannels
        if ismember(channel,uniqueChannelList)
            channelInds = find(tempChannelList==channel);
            for i = channelInds
                spikes(structInd).timestamps = rawSpikes.(rawSpikesFieldNames{i});
                spikes(structInd).channel = str2num(rawSpikesFieldNames{i,1}(5:6));
                spikes(structInd).sort = str2num(rawSpikesFieldNames{i,1}(8));
                structInd = structInd + 1;
            end
        else
            spikes(structInd).timestamps = [];
            spikes(structInd).channel = channel;
            spikes(structInd).sort = 0;
            structInd = structInd + 1;
        end
    end
...
%% Preallocate Data Struct
    ...
    %Data
    Data = struct('trialNum',[],'trialName','','trialStatus',[],'targetData',[],'stateData',stateData,'Decoder',Decoder);
...
for trial = 1:numTrials
    ...
    %Get Spike Data
        if getSpikes == true
            ...
            [binTimes,channelSpikes,allChannelSpikeBins,allSortSpikeBins] = getBinnedSpikesNigel(binWidth,spikes,stateTransitions,channelList,numChannels,numSorts,channelSpikes,'right');
            %Exclude Channels
